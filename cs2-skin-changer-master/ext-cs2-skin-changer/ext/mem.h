...
public:
    ...
    void Read_s(uintptr_t address, void* buff, size_t size) const
    {
        ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(address), &buff, size, nullptr);
    }
    ...
    void Write_s(uintptr_t address, void* value, size_t s) const 
    {
        WriteProcessMemory(hProcess, reinterpret_cast<LPVOID>(address), &value, s, nullptr);
    }

    void Clear8(uintptr_t address, size_t s)
    {
        for (size_t i = 0; i < s / 8; i++)
        {
            Write<uint64_t>(address + (i * 8), NULL);
        }
    }
